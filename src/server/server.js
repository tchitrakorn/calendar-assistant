...
function extractEventTimes(events) {
...
app.get('/explore', [
  ...
  body('preferences').custom(v.validateExplorePreferences)
], async (req, res) => {
  ...
  try {
    ...
    const calendarEvents = await itemController.track({ email: userEmail });

    if (!calendarEvents) {
      return res.status(500).send({ error: 'Error fetching calendar events' });
    }

    const eventTimes = extractEventTimes(calendarEvents.allEvents);
