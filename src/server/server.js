...
app.get(
  ...
  async (req, res) => {
    ...
    if (!expressValidatorErrors.isEmpty()) {
      return res
        .status(400)
        .send({ errors: expressValidatorErrors.array() }) // 400 for bad input
    }
...
app.get('/analytics', body('orgId').custom(v.validateOrgId), (req, res) => {
  const errors = validationResult(req)
  if (!errors.isEmpty()) {
